...
EXE_FILE_NAME=${EXE_FILE_NAME:-$(which tsbs_load_greenplum)}
if [[ -z "$EXE_FILE_NAME" ]]; then
    echo "tsbs_load_greenplum not available. It is not specified explicitly and not found in \$PATH"
    exit 1
fi

# Load parameters - common
DATA_FILE_NAME=${DATA_FILE_NAME:-greenplum-data.gz}
...
USE_HYPERTABLE=${USE_HYPERTABLE:-false}
...
cat ${DATA_FILE} | gunzip | $EXE_FILE_NAME \
                                ...
                                --partition-timestamp-interval="${PARTITION_TIMESTAMP_INTERVAL}" \
                                --partition-timestamp-start="${PARTITION_TIMESTAMP_START}" \
                                --partition-timestamp-end="${PARTITION_TIMESTAMP_END}" 
